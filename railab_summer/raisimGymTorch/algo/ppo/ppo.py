...
class PPO:
    ...
    def rew_log(self, actor_obs, value_obs, log_this_iteration, rews, name, update):
        last_values = self.critic.predict(torch.from_numpy(value_obs).to(self.device))
        ...
        if log_this_iteration:
            self.tot_timesteps += self.num_transitions_per_env * self.num_envs
            self.writer.add_scalar('Peformance/{}'.format(name), rews, update)

    def rewt_log(self, actor_obs, value_obs, log_this_iteration, rews, update):
        last_values = self.critic.predict(torch.from_numpy(value_obs).to(self.device))
        ...
        if log_this_iteration:
            self.tot_timesteps += self.num_transitions_per_env * self.num_envs
            self.writer.add_scalar('Average Performance', rews, update)

    def act_log(self, actor_obs, value_obs, log_this_iteration, acts, name, update):
        last_values = self.critic.predict(torch.from_numpy(value_obs).to(self.device))
        ...
        if log_this_iteration:
            self.tot_timesteps += self.num_transitions_per_env * self.num_envs
            self.writer.add_scalar('Action/action{}'.format(name), acts, update)
