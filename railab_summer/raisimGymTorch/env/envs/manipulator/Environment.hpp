...
class ENVIRONMENT : public RaisimGymEnv {
 ...
 public:
  ...
  float stepran(const Eigen::Ref<EigenVec>& action, const Eigen::Ref<EigenVec>& random_sample) {
    ...
    ball_->setPosition(ranval_[0], ranval_[1], ranval_[2]);
    ball_->setExternalForce(0, {0, 0, -0.005*9.81});

    // Eigen::Vector3d ee_Pos, ee_Vel;
    ...
    rewards_.record("pos_error", exp(-(ee_Pos__-ranval_.head(3)).squaredNorm()));
    rewards_.record("vel_error", exp(-(ee_Vel__-ranval_.tail(3)).squaredNorm()));
  ...
  void updateObservation() {
    ...
    anymal_->getFrameVelocity("ee_link", ee_Vel_);

    for (int i=0; i<3; i++){
      ee_Pos__[i] = ee_Pos_[i];
      ee_Vel__[i] = ee_Vel_[i];
    }
 ...
 private:
  ...
  Eigen::Vector3d bodyLinearVel_, bodyAngularVel_, ee_Pos__, ee_Vel__;
  raisim::Vec<3> ee_Vel_, ee_Pos_;
