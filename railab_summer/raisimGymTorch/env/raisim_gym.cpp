...
PYBIND11_MODULE(RAISIMGYM_TORCH_ENV_NAME, m) {
  py::class_<VectorizedEnvironment<ENVIRONMENT>>(m, RSG_MAKE_STR(ENVIRONMENT_NAME))
    ...
    .def("testob", &VectorizedEnvironment<ENVIRONMENT>::testob)
    .def("step", &VectorizedEnvironment<ENVIRONMENT>::step)
    .def("stepnew", &VectorizedEnvironment<ENVIRONMENT>::stepnew)
    .def("stepran", &VectorizedEnvironment<ENVIRONMENT>::stepran)
    .def("rewards_set", &VectorizedEnvironment<ENVIRONMENT>::rewards_set)
