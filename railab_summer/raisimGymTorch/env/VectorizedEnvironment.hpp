...
#pragma omp parallel for schedule(auto)
  ...
  void testob(Eigen::Ref<EigenRowMajorMat> &ob, bool updateStatistics, float &ran_x, float &ran_y, float &ran_yaw, float &j1, float &j2, float &j3, float &j4, float &j5, float &j6) {
#pragma omp parallel for schedule(auto)
    for (int i = 0; i < num_envs_; i++)
      environments_[i]->testob(ob.row(i), ran_x, ran_y, ran_yaw, j1, j2, j3, j4, j5, j6);
...
#pragma omp parallel for schedule(auto)
  ...
  void stepnew(Eigen::Ref<EigenRowMajorMat> &action,
            Eigen::Ref<EigenVec> &reward,
            Eigen::Ref<EigenBoolVec> &done,
            Eigen::Ref<EigenVec> &ranval) {
#pragma omp parallel for schedule(auto)
    for (int i = 0; i < num_envs_; i++)
      perAgentStepnew(i, action, reward, done, ranval);
  }

  void stepran(Eigen::Ref<EigenRowMajorMat> &action,
            Eigen::Ref<EigenVec> &reward,
            Eigen::Ref<EigenBoolVec> &done,
            Eigen::Ref<EigenRowMajorMat> &ranset) {
#pragma omp parallel for schedule(auto)
    for (int i = 0; i < num_envs_; i++)
      perAgentStepran(i, action, reward, done, ranset);
  }

  void rewards_set(Eigen::Ref<EigenVec> &reward,
                  Eigen::Ref<EigenBoolVec> &done,
                  int &k) {
#pragma omp parallel for schedule(auto)
    for (int i = 0; i < num_envs_; i++)
      perAgentReward(i, reward, done, k);
  }
...
#pragma omp parallel for schedule(auto)
  ...
  inline void perAgentStepnew(int agentId,
                           Eigen::Ref<EigenRowMajorMat> &action,
                           Eigen::Ref<EigenVec> &reward,
                           Eigen::Ref<EigenBoolVec> &done,
                           Eigen::Ref<EigenVec> &ranval) {
    reward[agentId] = environments_[agentId]->stepnew(action.row(agentId), ranval[agentId]);
    rewardInformation_[agentId] = environments_[agentId]->getRewards().getStdMap();
...
inline void perAgentStepran(int agentId,
                           Eigen::Ref<EigenRowMajorMat> &action,
                           Eigen::Ref<EigenVec> &reward,
                           Eigen::Ref<EigenBoolVec> &done,
                           Eigen::Ref<EigenRowMajorMat> &ranset) {
    reward[agentId] = environments_[agentId]->stepran(action.row(agentId), ranset.row(agentId));
    rewardInformation_[agentId] = environments_[agentId]->getRewards().getStdMap();
  ...
  inline void perAgentReward(int agentId,
                           Eigen::Ref<EigenVec> &reward,
                           Eigen::Ref<EigenBoolVec> &done,
                           int &k) {
    reward[agentId] = environments_[agentId]->rewards_set(k);
    rewardInformation_[agentId] = environments_[agentId]->getRewards().getStdMap();
